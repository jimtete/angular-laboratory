...
public sealed class CampaignUsersController : ControllerBase
{
    ...
    [HttpDelete("{username}")]
    public async Task<ActionResult<ApiResponse<bool>>> RemovePlayerFromCampaign(
        Guid campaignId,
        string username,
        CancellationToken cancellationToken)
    {
        ...
        var result = await _campaignParticipationInviteService.RemovePlayerFromCampaignAsync(
            userId.Value,
            campaignId,
            username,
            cancellationToken);
        ...
        return result.StatusCode switch
        {
            ...
            {
                StatusCode = StatusCodes.Status200OK,
                Message = "Campaign player removed successfully.",
                Data = true
            }),
