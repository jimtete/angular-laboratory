...
namespace LearningLab.Controllers.Campaigns;
...
public sealed class CampaignsController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<IReadOnlyList<CampaignResponse>>>> FetchJoinedCampaigns(
        CancellationToken cancellationToken)
    {
        var userId = SessionHelper.GetUserId(User);
        ...
        var result = await _campaignService.GetJoinedCampaignsAsync(
            userId.Value,
            cancellationToken);
        ...
        return result.StatusCode switch
        {
            ...
            ApplicationStatusCode.CampaignPlayerRoleRequired => StatusCode(
                StatusCodes.Status403Forbidden,
                new ApiResponse<IReadOnlyList<CampaignResponse>>
                {
                    StatusCode = StatusCodes.Status403Forbidden,
                    Message = "Only users with the Player role can fetch joined campaigns.",
                    Data = null
                }),
            _ => StatusCode(
                StatusCodes.Status500InternalServerError,
